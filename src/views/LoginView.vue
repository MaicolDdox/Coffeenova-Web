  ...
  <div class="min-h-screen bg-white text-[#291900] flex items-center justify-center relative overflow-hidden">
    ...
    <div class="relative w-full max-w-md p-8 rounded-3xl border border-[#e2e2e2] bg-white shadow-xl shadow-[#deb67e]/20 space-y-6">
      <div class="text-center space-y-1">
        ...
        <p class="text-sm text-[#7a7672]">Bienvenido de vuelta a CoffeeStore</p>
      </div>

      <form @submit.prevent="onSubmit" class="space-y-4">
        <div class="space-y-1">
          <label class="text-sm text-amber-200/80">Correo</label>
          <input
            v-model="email"
            type="email"
            required
            :class="inputClass"
            placeholder="demo@coffeestore.com"
          />
        </div>
        <div class="space-y-1">
          <label class="text-sm text-amber-200/80">Contrasena</label>
          <input
            v-model="password"
            type="password"
            required
      ...
      <p class="text-center text-sm text-[#7a7672]">
        No tienes cuenta?
        <router-link class="text-[#291900] font-semibold hover:underline" to="/register">Crear cuenta</router-link>
      </p>
    </div>
  </div>
</template>
...
const onSubmit = async () => {
  ...
  if (ok) router.push('/')
  else error.value = message || 'Credenciales invalidas'
}
</script>
